...
class MCTSNode:
    ...
    def select(self, final=False) -> tuple[int, int]:
        ...
        if final:
            return max(self.children.keys(), key=lambda a: self.children[a].times_chosen)

        untested_actions = [(cx, cy) for (cx, cy), child in self.children.items() if child.times_chosen == 0]
        if untested_actions:
            return random.choice(untested_actions)
    ...
    def simulate(self) -> Colour:
        if not self.terminal:
            ...
            game = Game(RandomPlayer(), RandomPlayer(), self.board.duplicate(), self.current_player)
            game.run()
            return game.winner
        else:
    ...
    def backpropagate(self, winner: Colour) -> None:
        ...
        self.value += (result - self.value) / self.times_chosen
